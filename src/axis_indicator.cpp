...
AxisIndicator::AxisIndicator(OSGViewerWidget *osg_viewer_widget):osg_viewer_widget_(osg_viewer_widget)
{
    toggleHidden();
}
...
void AxisIndicator::updateImpl(){
    ...
    content_root_->addChild(OSGUtility::drawArrow(bounding_box._min, bounding_box._min+up_vector*up_len, cylinder_thickness*1.01, osg::Vec4(1.0f, 1.0f, 0.0f, 1.0f)));
    
    double x_len = bounding_box.xMax() - bounding_box.xMin();
    content_root_->addChild(OSGUtility::drawArrow(bounding_box._min, bounding_box._min+osg::Vec3(x_len, 0, 0), cylinder_thickness, osg::Vec4(1.0f, 0.0f, 0.0f, 1.0f)));
    
    double y_len = bounding_box.yMax() - bounding_box.yMin();
	content_root_->addChild(OSGUtility::drawArrow(bounding_box._min, bounding_box._min+osg::Vec3(0, y_len, 0), cylinder_thickness, osg::Vec4(0.0f, 1.0f, 0.0f, 1.0f)));
    
    double z_len = bounding_box.zMax() - bounding_box.zMin();
	content_root_->addChild(OSGUtility::drawArrow(bounding_box._min, bounding_box._min+osg::Vec3(0, 0, z_len), cylinder_thickness, osg::Vec4(0.0f, 0.0f, 1.0f, 1.0f)));
